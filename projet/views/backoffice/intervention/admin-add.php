...
$errors = [];
$data = ['type'=>'','date_intervention'=>'','duree'=>'','niveau_urgence'=>'3','chirurgien'=>'','salle'=>'','description'=>''];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $result = $ctrl->addIntervention($data);
    if ($result['success']) {
...
<head>
    ...
    <title>Ajouter Intervention - MedChain</title>
...
<div class="dashboard-container">
    ...
    <main class="dashboard-main">
        <div class="card">
            <div class="card-header"><h2><i class="bi bi-plus-circle"></i> Ajouter une Intervention</h2></div>
            <div class="card-body">
                <?php if (!empty($errors)): ?>
                    <div class="alert-error"><i class="bi bi-exclamation-triangle-fill"></i> <?= implode('<br>', $errors) ?></div>
                <?php endif; ?>
                <form method="POST" data-form-type="intervention">
                    <div class="form-row">
                        <div class="form-group"><label><i class="bi bi-tag"></i> Type</label><input type="text" name="type" class="form-control" value="<?= htmlspecialchars($data['type']) ?>" placeholder="Ex: Chirurgie cardiaque"></div>
                    ...
                    <div class="form-row">
                        <div class="form-group"><label><i class="bi bi-clock"></i> Durée (min)</label><input type="text" name="duree" class="form-control" value="<?= htmlspecialchars($data['duree']) ?>" placeholder="60"></div>
                        <div class="form-group"><label><i class="bi bi-exclamation-triangle"></i> Niveau d'urgence</label>
                            <select name="niveau_urgence" class="form-control">
                                ...
                                <option value="1" <?= $data['niveau_urgence']=='1'?'selected':'' ?>>1 - Faible</option>
                                <option value="2" <?= $data['niveau_urgence']=='2'?'selected':'' ?>>2 - Modéré</option>
                                <option value="3" <?= $data['niveau_urgence']=='3'?'selected':'' ?>>3 - Élevé</option>
                                <option value="4" <?= $data['niveau_urgence']=='4'?'selected':'' ?>>4 - Urgent</option>
                                <option value="5" <?= $data['niveau_urgence']=='5'?'selected':'' ?>>5 - Critique</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group"><label><i class="bi bi-person"></i> Chirurgien</label><input type="text" name="chirurgien" class="form-control" value="<?= htmlspecialchars($data['chirurgien']) ?>" placeholder="Dr. Nom"></div>
                        <div class="form-group"><label><i class="bi bi-door-open"></i> Salle</label><input type="text" name="salle" class="form-control" value="<?= htmlspecialchars($data['salle']) ?>" placeholder="Salle A1"></div>
                    </div>
                    <div class="form-group"><label><i class="bi bi-card-text"></i> Description</label><textarea name="description" class="form-control" placeholder="Description de l'intervention..."><?= htmlspecialchars($data['description']) ?></textarea></div>
