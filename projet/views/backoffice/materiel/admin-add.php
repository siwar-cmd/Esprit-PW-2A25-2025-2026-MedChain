...
$errors = [];
$data = ['nom'=>'','categorie'=>'','disponibilite'=>'Disponible','statutSterilisation'=>'Stérilisé','nombreUtilisationsMax'=>'0','nombreUtilisationsActuelles'=>'0'];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = $_POST;
    $result = $ctrl->addMateriel($data);
    if ($result['success']) { $_SESSION['success_message'] = $result['message']; header('Location: admin-index.php'); exit; }
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}:root{--green:#1D9E75;--green-dark:#0F6E56;--navy:#1E3A52;--gray-500:#6B7280;--gray-200:#E5E7EB;--white:#fff;--radius-md:12px;--radius-xl:28px;--shadow-sm:0 1px 3px rgba(0,0,0,.08);--shadow-green:0 8px 30px rgba(29,158,117,.22)}
        ...
        .btn-primary{background:linear-gradient(135deg,var(--green),var(--green-dark));color:white;box-shadow:0 3px 12px rgba(29,158,117,.30)}.btn-primary:hover{transform:translateY(-2px)}
...
<body>
<div class="card">
    <div class="card-header"><h2><i class="bi bi-plus-circle"></i> Ajouter un Matériel</h2></div>
    <div class="card-body">
        ...
        <form method="POST" data-form-type="materiel">
            <div class="form-row">
                <div class="form-group"><label><i class="bi bi-box"></i> Nom</label><input type="text" name="nom" class="form-control" value="<?=htmlspecialchars($data['nom'])?>" placeholder="Nom du matériel"></div>
                <div class="form-group"><label><i class="bi bi-tag"></i> Catégorie</label><input type="text" name="categorie" class="form-control" value="<?=htmlspecialchars($data['categorie'])?>" placeholder="Ex: Instruments"></div>
            </div>
            <div class="form-row">
            ...
            <div class="form-row">
                <div class="form-group"><label><i class="bi bi-arrow-up-circle"></i> Utilisations Max</label><input type="text" name="nombreUtilisationsMax" class="form-control" value="<?=htmlspecialchars($data['nombreUtilisationsMax'])?>" placeholder="0"></div>
                <div class="form-group"><label><i class="bi bi-arrow-repeat"></i> Utilisations Actuelles</label><input type="text" name="nombreUtilisationsActuelles" class="form-control" value="<?=htmlspecialchars($data['nombreUtilisationsActuelles'])?>" placeholder="0"></div>
            </div>
