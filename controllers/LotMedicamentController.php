...
class LotMedicamentController {
    ...
    public function createLotMedicament($data): array {
        try {
            ...
            $sql = 'INSERT INTO lot_medicament (nom_medicament, type_medicament, date_fabrication, date_expiration, quantite_initial, description, is_sensible) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)';
            $req = $this->pdo->prepare($sql);
            $success = $req->execute([
    ...
    public function updateLotMedicament($id, $data): array {
        try {
            ...
            $allowedFields = ['nom_medicament', 'type_medicament', 'date_fabrication', 'date_expiration', 'quantite_initial', 'description', 'is_sensible'];
