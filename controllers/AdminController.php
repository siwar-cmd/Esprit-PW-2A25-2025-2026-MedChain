...
class AdminController {
    ...
    public function createUser($data): array {
        try {
            ...
            $validRoles = ['admin', 'user', 'medecin', 'patient'];
            if (!in_array($data['role'], $validRoles)) {
                return ["success" => false, "message" => "Rôle invalide : " . $data['role']];
            }
            ...
            // Spécialité uniquement pour les médecins
            $specialite = null;
            if ($data['role'] === 'medecin' && !empty($data['specialite'])) {
                $specialite = htmlspecialchars(trim($data['specialite']));
            }
            
            $sql = "INSERT INTO utilisateur (nom, prenom, email, mot_de_passe, role, statut, specialite, date_inscription) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
            $req = $this->pdo->prepare($sql);
            $success = $req->execute([
            ...
            if ($success) {
                error_log("✅ Utilisateur créé : {$data['email']} (rôle: {$data['role']})");
                return ["success" => true, "message" => "Utilisateur créé avec succès"];
            } else {
                $errorInfo = $req->errorInfo();
                error_log("❌ Erreur SQL createUser : " . print_r($errorInfo, true));
                return ["success" => false, "message" => "Erreur SQL : " . $errorInfo[2]];
            }
        } catch (Exception $e) {
            error_log("🔥 Exception createUser: " . $e->getMessage());
            return ["success" => false, "message" => "Erreur : " . $e->getMessage()];
        }
    }
    ...
    public function updateUser($userId, $data): array {
        try {
            ...
            $allowedFields = ['nom', 'prenom', 'email', 'role', 'statut', 'specialite'];
