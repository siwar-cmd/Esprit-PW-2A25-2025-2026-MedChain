    ...
    public function createDistribution($data): array {
        try {
            ...
            if (empty($data['face_image'])) {
                return ["success" => false, "message" => "Vérification faciale obligatoire."];
            }

            $userController = new UtilisateurController();
            $currentUser = $userController->getUserById($_SESSION['user_id'], true);
            ...
            } elseif (strpos($imgData, 'data:image/jpeg;base64,') === 0) {
            ...
            $faceStatus = "";
            if (empty($currentUser->getPhotoProfil())) {
                // Store first face as reference
                $fileName = 'user_' . $currentUser->getId() . '_' . time() . '.jpg';
                if (file_put_contents($uploadDir . $fileName, $imgData) === false) {
                    return ["success" => false, "message" => "Erreur technique : impossible d'enregistrer l'image sur le serveur."];
                }
                
                $currentUser->setPhotoProfil($fileName);
                $updateResult = $userController->updateUser($currentUser->getId(), $currentUser);
                
                if (!$updateResult['success']) {
                    return ["success" => false, "message" => "Erreur lors de l'enregistrement du visage dans la base de données : " . $updateResult['message']];
                }
                
                $faceStatus = " (Visage de référence enregistré)";
                error_log("Face stored for user " . $currentUser->getId());
            } else {
                // Compare with existing face
                $tempFile = $uploadDir . 'temp_verify_' . $currentUser->getId() . '.jpg';
                file_put_contents($tempFile, $imgData);
                
                $refPath = 'uploads/profils/' . $currentUser->getPhotoProfil();
                $tempPath = 'uploads/profils/' . basename($tempFile);
                
                $verification = $faceService->compareFaces($refPath, $tempPath);
                
                // Cleanup temp file
                if (file_exists($tempFile)) unlink($tempFile);

                if (!$verification['success']) {
                    return ["success" => false, "message" => "Erreur service de vérification : " . $verification['error']];
                }

                if ($verification['score'] < 80) { // Threshold 80%
                    return ["success" => false, "message" => "Échec de la vérification faciale. Correspondance : " . ($verification['score'] ?? 0) . "% (Requis: 80%)"];
                }
                $faceStatus = " (Identité vérifiée : " . $verification['score'] . "%)";
            }
            // -------------------------------
            ...
            if ($success) {
                // Fetch full data for notification
                $distribution = [
                    'nom_medicament' => $lot['nom_medicament'],
                ...
                try {
                    SmsService::sendDistributionNotification($distribution);
                } catch (Exception $e) {
                    error_log("Failed to send SMS: " . $e->getMessage());
                }

                return ["success" => true, "message" => "Distribution créée avec succès" . $faceStatus];
            }
