...
.stat-card{background:var(--white);border-radius:24px;padding:24px;display:flex;flex-direction:column;gap:12px;border:1px solid rgba(29,158,117,.1);box-shadow:0 4px 20px rgba(0,0,0,0.03);transition:all .3s ease;position:relative;overflow:hidden;}
.stat-card::after{content:'';position:absolute;top:0;right:0;width:80px;height:80px;background:linear-gradient(135deg,transparent 50%,rgba(29,158,117,0.03) 50%);border-radius:0 24px 0 100%;}
.stat-card:hover{transform:translateY(-5px);box-shadow:0 12px 30px rgba(29,158,117,0.12);border-color:var(--green);}
.stat-icon{width:54px;height:54px;border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:4px;}
.stat-icon.primary{background:linear-gradient(135deg,rgba(29,158,117,.15),rgba(29,158,117,.05));color:var(--green);}
.stat-icon.success{background:linear-gradient(135deg,rgba(34,197,94,.15),rgba(34,197,94,.05));color:#22C55E;}
.stat-icon.warning{background:linear-gradient(135deg,rgba(245,158,11,.15),rgba(245,158,11,.05));color:#F59E0B;}
.stat-icon.info{background:linear-gradient(135deg,rgba(59,130,246,.15),rgba(59,130,246,.05));color:#3B82F6;}
.stat-content h3{font-family:'Syne',sans-serif;font-size:32px;font-weight:800;color:var(--navy);line-height:1;}
.stat-content p{font-size:14px;font-weight:600;color:var(--gray-500);text-transform:uppercase;letter-spacing:0.5px;}
...
<div class="dashboard-container">
  <aside class="dashboard-sidebar">
    ...
    <nav class="dashboard-nav">
      ...
      <div class="nav-title">Stock & Pharmacie</div>
      <a href="../lot_medicament/admin-index.php" class="dashboard-nav-item"><i class="bi bi-pills"></i> Lots Médicaments</a>
      <a href="../distribution/admin-index.php" class="dashboard-nav-item"><i class="bi bi-truck-loading"></i> Distributions</a>

      <div class="nav-title">Flotte &amp; Missions</div>
      <a href="../missions/admin-demandes.php" class="dashboard-nav-item nav-sub"><i class="bi bi-send-plus-fill"></i> Demandes</a>
      <a href="admin-index.php" class="dashboard-nav-item active nav-sub"><i class="bi bi-truck-front-fill"></i> Ambulance</a>
      <a href="../missions/admin-index.php" class="dashboard-nav-item nav-sub"><i class="bi bi-geo-alt-fill"></i> Registre Missions</a>

      <div class="nav-title">Bloc Opération</div>
      <a href="../../admin_map.html" class="dashboard-nav-item"><i class="bi bi-map-fill"></i> Carte 3D</a>
      <a href="../bloc-operation/materiel-index.php" class="dashboard-nav-item"><i class="bi bi-tools"></i> Matériel</a>
      <a href="../bloc-operation/intervention-index.php" class="dashboard-nav-item"><i class="bi bi-heart-pulse-fill"></i> Interventions</a>
  ...
  <main class="dashboard-main">
    ...
    <div class="charts-grid">
        ...
        <div class="chart-container">
          <h4><i class="bi bi-bar-chart-line-fill"></i> État des Véhicules</h4>
          <div class="chart-wrapper" style="width:100%; height:250px;"><canvas id="statutChart"></canvas></div>
        </div>
    </div>
    ...
    <div class="card">
      ...
      <form method="GET" class="filter-bar">
        <input type="text" name="search" class="search-input" placeholder="🔍  Rechercher immatriculation, modèle…" value="<?= htmlspecialchars($search) ?>">
        <select name="statut">
        ...
        <a href="admin-index.php" class="btn btn-outline btn-filter"><i class="bi bi-arrow-counterclockwise"></i></a>
        <?php endif; ?>
      </form>
      ...
      <div class="card-body" style="padding:0;">
        ...
        <!-- Pagination -->
        <?php if($totalPages > 1): ?>
        <div style="display:flex;justify-content:center;gap:8px;padding:20px;border-top:1px solid var(--gray-200);">
          <a href="?page=<?= max(1, $page-1) ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filterStatut) ?>&disponible=<?= $filterDispo ?>" 
             class="btn btn-outline btn-sm <?= $page<=1?'disabled':'' ?>" style="<?= $page<=1?'opacity:0.5;pointer-events:none;':'' ?>">
            <i class="bi bi-chevron-left"></i> Précédent
          </a>
          <?php for($i=1; $i<=$totalPages; $i++): ?>
            <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filterStatut) ?>&disponible=<?= $filterDispo ?>" 
               class="btn btn-sm <?= $i==$page?'btn-primary':'btn-outline' ?>">
              <?= $i ?>
            </a>
          <?php endfor; ?>
          <a href="?page=<?= min($totalPages, $page+1) ?>&search=<?= urlencode($search) ?>&statut=<?= urlencode($filterStatut) ?>&disponible=<?= $filterDispo ?>" 
             class="btn btn-outline btn-sm <?= $page>=$totalPages?'disabled':'' ?>" style="<?= $page>=$totalPages?'opacity:0.5;pointer-events:none;':'' ?>">
...
<div class="modal-overlay" id="createModal">
  <div class="modal">
    ...
    <form method="POST" novalidate>
      ...
      <div class="form-row">
        <div class="form-group"><label>Immatriculation *</label><input type="text" name="immatriculation" data-required placeholder="Ex: TU-123-456"></div>
        <div class="form-group"><label>Modèle *</label><input type="text" name="modele" data-required placeholder="Ex: Mercedes Sprinter"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Statut</label>
          <select name="statut"><option>En service</option><option>Hors service</option><option>En maintenance</option></select>
        </div>
        <div class="form-group"><label>Capacité (personnes)</label><input type="text" name="capacite" value="2" placeholder="Ex: 4"></div>
      </div>
...
<div class="modal-overlay" id="editModal">
  <div class="modal">
    ...
    <form method="POST" novalidate>
      ...
      <div class="form-row">
        <div class="form-group"><label>Immatriculation *</label><input type="text" name="immatriculation" id="editImmat" data-required></div>
        <div class="form-group"><label>Modèle *</label><input type="text" name="modele" id="editModele" data-required></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Statut</label>
          <select name="statut" id="editStatut"><option>En service</option><option>Hors service</option><option>En maintenance</option></select>
        </div>
        <div class="form-group"><label>Capacité</label><input type="text" name="capacite" id="editCapacite" placeholder="Ex: 4"></div>
      </div>
...
const ctxS = document.getElementById('statutChart').getContext('2d');
new Chart(ctxS, {
    type: 'bar',
    data: {
        labels: ['Flotte'],
        datasets: [
            { label: 'En service', data: [<?= $stats['enService'] ?>], backgroundColor: '#1D9E75', borderRadius: 8 },
            { label: 'Maintenance', data: [<?= $stats['enMaintenance'] ?>], backgroundColor: '#F59E0B', borderRadius: 8 },
            { label: 'Hors service', data: [<?= $stats['horsService'] ?>], backgroundColor: '#EF4444', borderRadius: 8 }
        ]
    },
...
<?php include __DIR__ . '/../components/eda_widget.php'; ?>
