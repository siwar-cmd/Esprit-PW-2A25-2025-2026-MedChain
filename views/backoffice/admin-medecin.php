...
// Filtrage manuel : on ne garde que les médecins
$medecins = array_filter($allUsers, function($user) use ($search, $statut_filter) {
    // Ne garder que le rôle 'medecin'
    if (($user['role'] ?? '') !== 'medecin') return false;
    
    // Filtre par statut
    if ($statut_filter !== '' && ($user['statut'] ?? '') !== $statut_filter) return false;
    
    // Filtre par recherche (nom, prénom, email, spécialité)
    if ($search !== '') {
        $searchLower = strtolower($search);
        $nomPrenom = strtolower(($user['prenom'] ?? '') . ' ' . ($user['nom'] ?? ''));
        $email = strtolower($user['email'] ?? '');
        $specialite = strtolower($user['specialite'] ?? '');
        $telephone = strtolower($user['telephone'] ?? '');
        
        if (strpos($nomPrenom, $searchLower) === false && 
            strpos($email, $searchLower) === false &&
            strpos($specialite, $searchLower) === false &&
...
$medecins = array_values($medecins); // ré-indexation

// Tri des médecins
usort($medecins, function($a, $b) use ($sort_by, $sort_order) {
    $valueA = $a[$sort_by] ?? '';
    $valueB = $b[$sort_by] ?? '';
    
    if ($sort_by === 'date_inscription') {
...
$total_medecins = count($medecins);
$medecins_actifs = count(array_filter($medecins, fn($m) => ($m['statut'] ?? '') === 'actif'));
$medecins_inactifs = $total_medecins - $medecins_actifs;
...
<head>
    ...
    <title>Gestion des médecins - MedChain Admin</title>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .form-control, .form-select{padding:8px 12px;border:1px solid var(--gray-200);border-radius:8px;width:100%}
        .form-label{font-size:13px;margin-bottom:6px;display:block;color:var(--gray-700)}
        ...
        .specialite-badge{background:#e0f2fe;color:#0369a1;padding:4px 10px;border-radius:20px;font-size:0.8rem;display:inline-block}
        .status-badge.actif{background:#d1fae5;color:#065f46;padding:4px 10px;border-radius:20px;font-size:12px}
        .status-badge.inactif{background:#fee2e2;color:#991b1b;padding:4px 10px;border-radius:20px;font-size:12px}
        ...
        .col-md-2,.col-md-3,.col-md-5{flex:1;padding:8px;min-width:150px}
...
<div class="dashboard-container">
    ...
    <aside class="dashboard-sidebar">
        ...
        <nav class="dashboard-nav">
            ...
            <a href="admin-medecin.php" class="dashboard-nav-item active"><i class="bi bi-heart-pulse-fill"></i> Médecins</a>
    ...
    <main class="dashboard-main">
        <div class="page-header">
            <h1><i class="bi bi-heart-pulse-fill"></i> Gestion des médecins</h1>
            <a href="admin-create-user.php?role=medecin" class="btn-sm" style="background:var(--green);color:#fff;border:none;padding:10px 18px;border-radius:8px;text-decoration:none;">
        ...
        <div class="stats-cards">
            <div class="stat-card">
                <div class="stat-card-title">Total médecins</div>
                <div class="stat-card-value"><?= $total_medecins ?></div>
                <div class="stat-card-desc"> inscrits au total</div>
            </div>
            <div class="stat-card">
                <div class="stat-card-title">Médecins actifs</div>
                <div class="stat-card-value" style="color:#22C55E"><?= $medecins_actifs ?></div>
                <div class="stat-card-desc"> comptes actifs</div>
            </div>
            <div class="stat-card">
                <div class="stat-card-title">Médecins inactifs</div>
                <div class="stat-card-value" style="color:#EF4444"><?= $medecins_inactifs ?></div>
        ...
        <div class="card mb-4">
            <div class="card-header">
                ...
                <?php if ($search || $statut_filter): ?>
                    <a href="admin-medecin.php" class="filter-reset"><i class="bi bi-x-circle"></i> Réinitialiser</a>
                <?php endif; ?>
            </div>
            <div class="card-body">
                <form method="GET" class="row">
                    <div class="col-md-5">
                        <label class="form-label">Recherche par nom, email, spécialité ou téléphone</label>
                        <input type="text" name="search" class="form-control" value="<?= e($search) ?>" placeholder="Nom, prénom, email, spécialité, téléphone...">
                    </div>
                    <div class="col-md-3">
        ...
        <div class="card">
            <div class="card-header">
                <span><i class="bi bi-list-ul"></i> Liste des médecins</span>
                <span class="text-muted" style="font-size:13px"><?= count($medecins) ?> médecin(s) affiché(s)</span>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>Email</th>
                            <th><a href="?<?= http_build_query(array_merge($_GET, ['sort_by' => 'specialite', 'sort_order' => $sort_by === 'specialite' && $sort_order === 'ASC' ? 'DESC' : 'ASC'])) ?>">Spécialité <?= $sort_by === 'specialite' ? ($sort_order === 'ASC' ? '↑' : '↓') : '' ?></a></th>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                <td>#<?= e($m['id_utilisateur']) ?></td>
                                <td>
                                    <strong>Dr. <?= e($m['prenom'] . ' ' . $m['nom']) ?></strong>
                                    <?php if (!empty($m['date_naissance'])): ?><br><small class="text-muted">Né(e) le <?= date('d/m/Y', strtotime($m['date_naissance'])) ?></small><?php endif; ?>
                                </td>
                                <td><?= e($m['email']) ?></td>
                                <td><span class="specialite-badge"><?= e($m['specialite'] ?? 'Généraliste') ?></span></td>
                                <td><span class="status-badge <?= e($m['statut']) ?>"><?= $m['statut'] === 'actif' ? '✓ Actif' : '✗ Inactif' ?></span></td>
                                <td><?= date('d/m/Y', strtotime($m['date_inscription'])) ?></td>
                                <td class="btn-group">
                                    <a href="admin-edit.php?id=<?= e($m['id_utilisateur']) ?>" class="btn-sm btn-outline-primary" title="Modifier">
                                        <i class="bi bi-pencil-fill"></i> Modifier
                                    </a>
                                    <?php if ($m['id_utilisateur'] != $_SESSION['user_id']): ?>
                                        <?php if ($m['statut'] === 'actif'): ?>
                                            <form method="POST" action="admin-deactivate.php" onsubmit="return confirm('Désactiver ce médecin ? Il ne pourra plus se connecter.')" style="display:inline">
                                                <input type="hidden" name="user_id" value="<?= e($m['id_utilisateur']) ?>">
                                        ...
                                        <?php else: ?>
                                            <form method="POST" action="admin-activate.php" onsubmit="return confirm('Activer ce médecin ? Il pourra à nouveau se connecter.')" style="display:inline">
                                                <input type="hidden" name="user_id" value="<?= e($m['id_utilisateur']) ?>">
                                                <input type="hidden" name="csrf_token" value="<?= e($csrfToken) ?>">
                                        ...
                                        <?php endif; ?>
                                        <form method="POST" action="admin-delete.php" onsubmit="return confirm('⚠️ ATTENTION : Supprimer définitivement ce médecin ? Cette action est irréversible et supprimera toutes ses données associées.')" style="display:inline">
                                            <input type="hidden" name="user_id" value="<?= e($m['id_utilisateur']) ?>">
                                            <input type="hidden" name="csrf_token" value="<?= e($csrfToken) ?>">
