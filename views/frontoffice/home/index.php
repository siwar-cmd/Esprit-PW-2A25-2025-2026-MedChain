  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chat-input input:focus {
      border-color: var(--green);
      box-shadow: 0 0 0 2px rgba(29,158,117,.2);
    }
    .chat-input button {
      background: linear-gradient(135deg, var(--green), var(--green-dark));
      border: none;
    ...
    @media (max-width: 480px) {
      .chat-window { width: 320px; height: 460px; right: -10px; }
    }
  </style>
</head>
...
<header id="header">
  ...
  <div class="mc-branding">
    ...
    <div class="mc-actions">
      ...
      <?php else: ?>
        ...
        <button class="btn-solid-mc" onclick="openRoleModal()" style="border:none;cursor:pointer;">
  ...
  <div class="mc-mobile-nav" id="mobileNav">
    ...
    <div class="mc-mob-btns">
      ...
      <?php else: ?>
        <a href="../auth/login.php" class="btn-outline-mc">Connexion</a>
        <button class="btn-solid-mc" onclick="openRoleModal()" style="border:none;cursor:pointer;width:100%;">Inscription</button>
      <?php endif; ?>
    </div>
  </div>
...
<div class="chat-widget">
    ...
    <div id="chatWindow" class="chat-window closed">
        <div class="chat-header">
            ...
            <button id="chatClose" class="chat-close"><i class="bi bi-x-lg"></i></button>
        </div>
        <div id="chatMessages" class="chat-messages">
            <div class="message bot">
                <div class="avatar-small"><i class="bi bi-robot"></i></div>
                <div class="bubble">Bonjour ! 👋 Je suis l'assistant MedChain. Posez-moi vos questions sur la plateforme, la sécurité blockchain, les fonctionnalités ou l'inscription.</div>
            </div>
        </div>
        <div class="suggestions" id="suggestions">
            <button data-msg="Comment créer un compte ?">📝 Créer un compte</button>
            <button data-msg="Comment fonctionne la sécurité blockchain ?">🔒 Sécurité blockchain</button>
            <button data-msg="Quels sont les tarifs ?">💰 Tarifs</button>
            <button data-msg="Puis-je prendre un rendez-vous en ligne ?">📅 Rendez-vous</button>
        </div>
...
<script>
  ...
  (function() {
    ...
    function openChat() { windowEl.classList.remove('closed'); isOpen = true; }
    function closeChat() { windowEl.classList.add('closed'); isOpen = false; }
    ...
    function addMessage(text, isUser) {
      ...
      const avatar = isUser ? '<div class="avatar-small"><i class="bi bi-person-fill"></i></div>' : '<div class="avatar-small"><i class="bi bi-robot"></i></div>';
    ...
    function showTyping() {
      ...
      div.id = 'typingMsg';
      div.innerHTML = `<div class="avatar-small"><i class="bi bi-robot"></i></div><div class="typing-indicator"><div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div></div>`;
      messagesDiv.appendChild(div);
      messagesDiv.scrollTop = messagesDiv.scrollHeight;
    }
    ...
    async function sendMessage(message) {
      ...
      try {
        const response = await fetch('../../../controllers/public-chat.php', {
          method: 'POST',
        ...
        removeTyping();
        if (data.error) {
          addMessage('❌ Désolé, une erreur technique est survenue. Réessayez plus tard.', false);
        } else if (data.reply) {
          addMessage(data.reply, false);
        } else {
          addMessage('❌ Désolé, je n\'ai pas pu comprendre. Veuillez reformuler.', false);
        }
      } catch (err) {
        removeTyping();
        addMessage('❌ Impossible de contacter le serveur. Vérifiez votre connexion.', false);
      } finally {
        isLoading = false;
...
<div id="roleModal" style="display:none;position:fixed;inset:0;z-index:99999;align-items:center;justify-content:center;">
  ...
  <div id="roleModalCard" style="position:relative;z-index:2;width:100%;max-width:460px;margin:20px;background:#fff;border-radius:24px;box-shadow:0 24px 80px rgba(0,0,0,0.18);overflow:hidden;transform:scale(0.9);opacity:0;transition:all 0.3s cubic-bezier(.34,1.56,.64,1);">
    ...
    <div style="padding:0 32px 24px;text-align:center;">
      ...
      <a href="../auth/login.php" style="font-size:13px;color:#1D9E75;font-weight:600;text-decoration:none;">Se connecter</a>
    </div>
  </div>
</div>
...
<script>
  ...
  function selectRole(role) {
    window.location.href = '../auth/register.php?role=' + role;
  }
  // Close on Escape key
